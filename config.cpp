...
bool Config::getConfig(const char *FileName) {
    ...
    element = algorithm->FirstChildElement(CNS_TAG_VR);
    if (!element) {
        std::cout << "Warning! No '" << CNS_TAG_VR << "' element found in XML file." << std::endl;
        std::cout << "Value of '" << CNS_TAG_VR << "' was defined to default - 1" << std::endl;
        SearchParams[CN_SP_VR] = 1;
    } else {
        std::string check;
        stream << element->GetText();
        stream >> check;
        ...
        stream << element->GetText();
        stream >> SearchParams[CN_SP_VR];
        stream.str("");
        stream.clear();

        SearchParams[CN_SP_VR] = floor(SearchParams[CN_SP_VR]);
    }
