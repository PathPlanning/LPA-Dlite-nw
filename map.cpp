...
Map::Map(unsigned width, unsigned height, std::pair<unsigned, unsigned> start, std::pair<unsigned, unsigned> goal) {
    this->height = height;
    this->width = width;
    start_i = start.first;
    ...
    goal_i = goal.first;
    goal_j = goal.second;
    Grid.resize(height, std::vector<int>(width, CN_GC_NOOBS));
}
